...
class CFG:
    ...
    exps = ['exp047', 'exp058', 'exp065', 'exp073', 'exp076']
    #exps_tta = [False, False, True, True]
    exps_tta = [False, True, True, True, True]
...
def main():
    ...
    train_preds = train_df[feat_cols].mean(axis=1)
    test_preds = np.mean(test_preds, axis=0)
    
    train_df['preds'] = np.clip(train_preds, 0, 3)
