...
class CFG:
    ...
    epochs=25
    multi_gpu = False 
    batch_size=64  # batch size per GPUs


    # GradualWarmupSchedulerV2
    scheduler_params = {'lr_start': 7.5e-6, 'min_lr': 1e-6, 'lr_max': 1e-3  if multi_gpu else 1e-3  }
    ...
    half_century = True  # targetを半世紀ごとに分割, BかCでしか効果ない
...
for fold, (train_idx, val_idx) in enumerate(KFold(n_splits=CFG.n_fold, shuffle=True, random_state=CFG.seed).split(train_df['image'], train_df['target'])):
...
def get_transforms(*, data):
    ...
    if data == 'train':
        return T.Compose([
            ...
            T.RandomResizedCrop((CFG.size, CFG.size)),
            T.ToTensor(),
