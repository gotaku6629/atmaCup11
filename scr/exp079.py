...
class CustomModel(nn.Module):
    def __init__(self, cfg, pretrained=False, backbone=None, target_size=1):
        ...
        if 'swin' in self.cfg.model_name and 'tiny' in self.cfg.model_name:
            ...
            state_dict = torch.load('../esvit_out2/checkpoint.pth', map_location='cpu')['student']
        ...
        elif 'swin' in self.cfg.model_name and 'small' in self.cfg.model_name:
            self.model = swin_transfomer.SwinTransformer(
                img_size=CFG.size,
                in_chans=3,
                num_classes=1,
            ...
            self.n_features = self.model.head.in_features
            self.model.head = nn.Identity()
            state_dict = torch.load('../esvit_out3/checkpoint.pth', map_location='cpu')['student']
            state_dict = fix_model_state_dict(state_dict)
            self.model.load_state_dict(
                state_dict,
            )
...
def train_loop(folds, fold, backbone=None, phase='train'):
    ...
    if 'swin' in CFG.model_name:
        param_group = [
            ...
            {'params': model.model.layers[0].parameters(), 'lr': CFG.optimizer_params['lr']/100},
            {'params': model.model.layers[1].parameters(), 'lr': CFG.optimizer_params['lr']/50},
            {'params': model.model.layers[2].parameters(), 'lr': CFG.optimizer_params['lr']/20},
            {'params': model.model.layers[3].parameters(), 'lr': CFG.optimizer_params['lr']/10},
